...
import { emailTheClaimant } from "../../src/domains/infra/notice-email.ts"
...
function mailer(email: string | null = "ada@example.com") {
  ...
  const announce = emailTheClaimant({
...
describe("what lands in the claimant's inbox", () => {
  test("a nudge names the failure and the record to fix, in the diagnosis' own words", async () => {
    const app = mailer()
    await app.announce(announcement(APEX))
    ...
    expect(sent?.text).toContain("acme.com")
  })

  test("the day-six warning says the window closes tomorrow", async () => {
    const app = mailer()
    await app.announce(announcement({ kind: "expiring", diagnosis: APEX.diagnosis }))
  ...
  test("the proof email carries the date the proof is dated by", async () => {
    const app = mailer()
    await app.announce(announcement({ kind: "proved", provedAt: new Date("2026-08-24T12:00:00Z") }))
  ...
  test("the coexistence email says the other proof takes nothing away", async () => {
    const app = mailer()
    await app.announce(announcement({ kind: "coexistence" }))
