...
import { attestationRoutes } from "./api/attestation.routes.ts"
...
export function proofApp(module: ProofModule, session: SessionPlugin, config: ProofConfig) {
  return new Elysia({ name: "proof" })
    .use(proofLinkRoutes(module, session, config.appUrl))
    .use(attestationRoutes(module.getProof, config.appUrl))
}
