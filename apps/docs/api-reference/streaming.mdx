...
`502`, per [the three ways a read fails](/concepts/zone-reading#when-it-cannot-answer). The
stream never carries a half-read zone, and there is no error event to handle inside the loop.
