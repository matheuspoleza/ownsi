...
`status` is what the API reports. It is derived from the state and the last check, so a new rule
...
| `proved` | Verified, and dated by the moment it was verified. | The proof and its date. |
...
The first three carry `records` and `expiresAt`. The last three carry `endedAt` and no records —
there is nothing left to write in a DNS panel.

## What moves a claim

<Steps>
  <Step title="POST /api/domains — claim">
    Issues the token. Status `pending`, `waitEstimate` set to `first_check`, `expiresAt` seven days
    out. On a domain whose last claim has ended, this is also how you start again: a new token, and
    the old claim moves to `history`.
  </Step>
  <Step title="POST /api/domains/:id/verify — ask for a check now">
    Forces an attempt instead of waiting for the schedule. Rate limited per claim. Answers
    [`claim_ended`](/errors#claim_ended) once the claim is over.
  </Step>
  <Step title="POST /api/domains/:id/cancel">
    Ends the open claim. The token stops being accepted from that moment.
  </Step>
  <Step title="POST /api/domains/:id/archive">
    Leaves the list and ends any open claim. **Retracts nothing.**
  </Step>
</Steps>
