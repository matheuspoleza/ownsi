...
Proved claims are not swept on a schedule, but an attempt that runs against one can only move
`lastConfirmedAt` forward — it can never un-prove a domain by accident. `firstVerifiedAt` is written
