...
<CardGroup cols={3}>
  <Card title="Fix it" icon="pen">
    ...
    Primary action, prominent. `fix` is a real instruction. `expired_token` is the cheapest of
    them: the record is in the right place and only its value is stale.
  </Card>
...
Leaving it in place is the supported state. A later claim on the same name issues a new token, so
the record already in the zone comes back as
[`expired_token`](/diagnostics/catalogue#expired_token) rather than as nothing found: the person
edits one value instead of working out where the record goes again. Telling them to delete it turns
that one-line edit back into the whole task.

[Archiving does not retract a proof](/concepts/claim-lifecycle#archiving-is-not-deleting) either —
every proof keeps its date. It does take back the links published from it, which is the point of
archiving rather than a side effect of it.
