...
| `proved` | The date on `claim.endedAt`, and the domain's `firstVerifiedAt` if it proved before. |
| `expired` | The window closed. One button to claim again — a new token, one edit in the panel. |
| `canceled` | History. No action; an ended claim takes none. |
