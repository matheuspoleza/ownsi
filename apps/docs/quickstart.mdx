...
<Steps>
  ...
  <Step title="Claim the domain">
    ...
    <CodeGroup>
    ```ts TypeScript
    const { data: domain } = await api.domains.post({ domain: "acme.com" })
    const { data: claim } = await api.claims.post({ domainId: domain.id })
    ...
    <Note>
      Two calls, not one: a domain is a name on your account, and a claim is one attempt at
      proving it. The claim comes back with `verificationId` — the process that will do the
      reading.
    </Note>
  ...
  <Step title="Create the record in your DNS panel">
    ...
    | Value | the `records[0].value` from the previous step, verbatim |
  ...
  <Step title="Read the verification back">
    ...
    <CodeGroup>
    ```ts TypeScript
    const { data: checked } = await api.verifications({ id: claim.verificationId }).get()
