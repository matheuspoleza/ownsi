...
<Note>
  The API is not open yet. A generated client works today only against the zone read and the
  public proof page; every other route wants a session the app issues, and there are no public keys
  to stand in for one. See [Authentication](/api-reference/authentication).
</Note>
