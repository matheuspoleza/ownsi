...
date: 2026-08-19
decided-by: Matheus
resolved-in: ../prd.md §3.11 (Railway + Bun + Neon, single origin at the edge)
---
...
Accepted, and now **resolved**: the tech design closed it as **one long-running Bun service on
Railway with Neon Postgres, behind a single origin at the edge** — see `../prd.md` §3.11,
[ADR-0027](0027-single-origin-edge-proxy.md) and [ADR-0028](0028-in-process-scheduler.md). The survey
below is kept so the work is not repeated.
